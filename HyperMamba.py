...
class HFEBlock(nn.Module):
    def __init__(self, dim=48, num_heads=1, match_factor=1, ffn_expansion_factor=1, bias=True, attention_matching=True, ffn_matching=True, ffn_restormer=False):
...
class DepthSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        self.depthwise_conv = nn.Conv2d(
            in_channels,
            in_channels,  # Output channels are the same as input channels for depthwise
            kernel_size=kernel_size,
            stride=stride,
            padding=padding,
            groups=in_channels  # Groups equal to number of input channels for depthwise
        )
        
        # Pointwise convolution
        self.pointwise_conv = nn.Conv2d(
            in_channels,
            out_channels,  # Output channels for pointwise
            kernel_size=1  # Pointwise convolution has a kernel size of 1x1
        )
...
class Down(nn.Module):
    def __init__(self, dim):
        ...
        self.dim = dim
        hidden_dim = dim // 8
        self.head = nn.Conv2d(dim, hidden_dim, 1,1)
        self.layer_norm_01 = LayerNorm2D(hidden_dim)
        self.conv1 = nn.Sequential(
            SELayer(hidden_dim),
        )
        self.conv2 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1,groups=hidden_dim)
        self.conv3 = nn.Conv2d(hidden_dim, hidden_dim,1,1)
        self.layer_norm_02 = LayerNorm2D(hidden_dim)
        self.ss2d = SS2DBlock(hidden_dim)
        self.tail = nn.Sequential(nn.Conv2d(hidden_dim, dim * 2, 3,1,1))
        self.dwt = DWT()
    ...
    def forward(self, x):
        x = self.head(x)
        res1 = x
        x = self.layer_norm_01(x)
        x1,x2 = x,x
        x1 = self.ss2d(x1)
        x2 = self.conv1(x2)
        x = F.sigmoid(x1) * F.sigmoid(x2) + res1
        x = self.tail(x)
        x_LL, x_HL, x_LH, x_HH = self.dwt(x)
        return x_LL, x_HL, x_LH, x_HH
...
class Up(nn.Module):
    def __init__(self, dim):
        ...
        self.ss2d_01 = SS2DBlock(hidden_dim // 2)
        self.dsconv = DepthSeparableConv(hidden_dim // 2, hidden_dim // 2, 3, 1, 1)
        self.layer_norm_01= LayerNorm2D(hidden_dim)
        self.conv3x3 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1)
        
        self.layer_norm_02= LayerNorm2D(hidden_dim)
        self.mlp = FC(hidden_dim)
        
    
    def forward(self, x, hf_feats):
        x_LL, x_HL, x_LH, x_HH = hf_feats
        x = torch.cat([x, x_HL, x_LH, x_HH], dim=1)
        x = self.idwt(x)
        x = self.head(x)    
        
        res = x
        x1,x2 = torch.chunk(x, 2, dim=1)
        x1,x2 = self.ss2d_01(x1), self.dsconv(x2)
        x = torch.cat([x1, x2], dim=1)
        x = self.conv3x3(x) + res
        x = self.tail(x)
        return x
...
class BriBlock(nn.Module):
    def __init__(self,dim):
        super(BriBlock, self).__init__()
        self.dim = dim
        hidden_dim = dim // 8
        self.head = nn.Conv2d(dim * 3, hidden_dim, 1,1)
        self.tail = nn.Conv2d(hidden_dim, dim * 3, 3,1,1)
        
        self.conv5x5 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1,groups=hidden_dim)
        self.conv3x3 = nn.Conv2d(hidden_dim, hidden_dim, 1, 1, 0)
        
        self.conv3x3_d1 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1,dilation=1,groups=hidden_dim)
        self.conv3x3_d2 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 2,dilation=2,groups=hidden_dim)
        self.ss2d3 = SS2DBlock(hidden_dim)
    ...
    def forward(self, feats):
        ...
        x_LL, x_HL, x_LH, x_HH = feats
        x = torch.concat([x_HL, x_LH, x_HH], dim=1)
        x = self.head(x)
        res = x
        x1,x2 = self.conv5x5(x), self.conv3x3(x)
        x11 = self.conv3x3_d1(x1)
        x12 = self.conv3x3_d2(x1)
        x2 = self.ss2d3(self.layer_norm(x2))
        x = x11 * self.alpha * x2 + x12 * (1 - self.alpha) * x2
        x = self.tail(x + res)
        x_HL, x_LH, x_HH = torch.split(x, self.dim, dim=1)
        return [x_LL, x_HL, x_LH, x_HH]


class BriBlock_(nn.Module):
    def __init__(self,dim):
        super(BriBlock_, self).__init__()
        self.dim = dim
        hidden_dim = dim // 8
        self.head = nn.Conv2d(dim * 3, hidden_dim, 1,1)
        self.tail = nn.Conv2d(hidden_dim, dim * 3, 3,1,1)
...
class HyperMamba(nn.Module):
    def __init__(self,hsi_bands,upscale=4,dim=16,depth=3):
        super(HyperMamba, self).__init__()
        ...
        self.tail = nn.Sequential(
            nn.Conv2d(dim, dim, 5,1,2),
            nn.ReLU(),
            nn.Conv2d(dim, hsi_bands,3,1,1),
        )
        
        
        self.down_blocks = nn.ModuleList([Down(dim * (2 ** i)) for i in range(depth)])
        self.up_blocks = nn.ModuleList([Up(dim * 2**(depth - i)) for i in range(depth)])
        self.brid_blocks = nn.ModuleList([BriBlock(dim * 2**(i + 1)) for i in range(depth)])
        
    def forward(self, x):
        x = F.interpolate(x,scale_factor=self.upscale, mode='bicubic')
        res = x 
        x_LL_0, x_HL_0, x_LH_0, x_HH_0 = self.head(x)
        down_feats = [[ x_LL_0, x_HL_0, x_LH_0, x_HH_0]]
        for i in range(len(self.down_blocks)):
            x_LL, x_HL, x_LH, x_HH = self.down_blocks[i](down_feats[i][0])
            down_feats.append(self.brid_blocks[i]([x_LL, x_HL, x_LH, x_HH]))

        
        x = down_feats[-1][0]
        
        for i in range(len(self.up_blocks)):
            x = self.up_blocks[i](x, down_feats[-i-1])
        
        x = torch.concat([x, x_HL_0, x_LH_0, x_HH_0],dim=1)
